...
def compute_lyapunov_ellipse(c, m=1.0, l=1.0, b=0.5, g=9.81):
    ...
    P = np.array([[g / l * scale, 0], [0, 1.0 * scale]])
...
def main():
    ...
    parser.add_argument(
        "--output-dir", type=str, default=".", help="Output directory (default: current directory)"
    )
    ...
    ax.plot(
        ...
        "orange",
        linewidth=2,
        label="Trajectory B: Outside → Enters → Converges",
    )
    ...
    if TIKZPLOTLIB_AVAILABLE:
        tikzplotlib.save(
            tex_output_path, axis_width="\\figwidth", axis_height="\\figheight", strict=False
        )
