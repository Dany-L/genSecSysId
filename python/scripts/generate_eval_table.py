...
def build_rows(config: Dict[str, Any], with_divergence: bool, only: Optional[str]):
    ...
    for experiment in config["experiments"]:
        ...
        for run in experiment["runs"]:
            ...
            logger.info("  %-7s %s  %s=%.4f", model_name, run_id, criterion, nrmse)
...
def main() -> int:
    ...
    parser.add_argument("--decimals", type=int, default=2, help="decimals for the metrics")
    ...
    comment = (
        ...
        f"{divergence['padding']} zero-input steps"
    )
    tex = build_eval_table(rows, decimals=args.decimals, comment=comment)
