...
from sysid.utils import get_device, print_model_summary, set_seed
...
def main():
    ...
    try:
        ...
        if train_path.is_dir():
            ...
            train_inputs, train_outputs = DataLoader.load_from_csv(
                data_config.train_path, delimiter=","
            )
            val_inputs, val_outputs = DataLoader.load_from_csv(data_config.val_path, delimiter=",")
        ...
        logger.info(
            f"Train data loaded: inputs={train_inputs.shape}, outputs={train_outputs.shape}"
        )
        logger.info(
            f"Validation data loaded: inputs={val_inputs.shape}, outputs={val_outputs.shape}"
        )
    ...
    with mlflow.start_run(run_name=config.mlflow.run_name):
        ...
        trainer = Trainer(
            ...
            decay_regularization_weight=(
                getattr(config.training, "decay_regularization_weight", False)
                if config.training.use_custom_regularization
                else False
            ),
            regularization_decay_factor=getattr(
        ...
        if config.training.use_custom_regularization:
            ...
            if getattr(config.training, "decay_regularization_weight", False):
                ...
                logger.info(
                    f"  Decay factor: {getattr(config.training, 'regularization_decay_factor', 0.5)}"
                )
                logger.info(
                    f"  Min threshold: {getattr(config.training, 'min_regularization_weight', 1e-7):.2e}"
                )
            else:
